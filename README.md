...
面板之外还有一条入口：把某台机器人标记为**管理机器人**，之后直接用 Telegram
私聊它来托管其他机器人 —— 加一个机器人不必先打开面板。
...
- 非管理员发的消息不会被当成管理命令，也不会收到「你不是管理员」这类回复 ——
  那等于向探测者确认这台机器人有管理功能。它在对方眼里就是一台普通机器人

开启方式：面板 →「机器人」→ 该机器人的「设为管理」；再到「设置」页填上你自己的
数字 ID（把任意一条消息转发给 @userinfobot 就能看到）。
...
> 面板不是唯一入口：把某台机器人设为**管理机器人**后，这套流程可以在 Telegram 里
> 直接走完（`/add`），两步做的校验完全一样 —— 两条入口共用同一段代码。
...
已验证的是：单元测试（63 个用例，含 ReDoS 免疫与 UTF-16 偏移）、API 端点、
