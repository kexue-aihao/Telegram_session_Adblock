...
**升级不用手动改库。** 新版启动时会自己补上新增的表与列（旧库原地 `ALTER`），
已有数据原样保留。
...
面板之外还有一条入口：**单独绑定一台**管理机器人，私聊它就能增删托管其他机器人 ——
加一个机器人不必先打开面板。

它是一台**专职控制台**：需要在 @BotFather 单独申请，**不参与转发**。谁给它发消息
都不会变成话题，它也不需要建群、不需要群权限、不需要关 Privacy Mode。
它与托管机器人各用各的 token，在「机器人」页分成两区显示，也不计入仪表盘的机器人数量。

- `/start` 打开菜单：机器人列表、添加、系统概况、帮助
- `/add` 是引导式的：先贴 token（当场调 `getMe` 校验，并提示 Privacy Mode 是否关掉），
  再填群 ID（当场群体检，逐项列出还缺哪项权限）
- `/bots` 里能对任意一个托管机器人启停、重载、删除，删前有一次确认

**它在 Telegram 上是公开可私聊的** —— 任何人搜到用户名都能给它发消息，而它能增删
其他机器人。所以全部管理命令都以「发送者的用户 ID == 设置页里的 `adminTgUserId`」为前提：

- **没配置（0）时，所有管理命令一律拒绝** —— 不会把控制权交给第一个找到它的人
- 非管理员发来的消息既不会被当成管理命令，也不会收到「你不是管理员」这类回复 ——
  回那句等于向探测者确认这台机器人有管理功能。它在对方眼里就是一台不吭声的机器人
- 绑定是**唯一**的：换一台会把旧的解绑，旧的随即变回普通机器人

绑定方式：面板 →「机器人」→「管理机器人」区 →「绑定管理机器人」，贴上 token 即可
（不必建群）；再到「设置」页填上你自己的 Telegram 数字 ID（把任意一条消息转发给
@userinfobot 就能看到）。想把已经托管的一台改成控制台，在它的卡片上点「设为控制台」。
...
> 面板不是唯一入口：绑定一台**管理机器人**之后，这套流程可以在 Telegram 里
> 直接走完（`/add`）—— 两条入口共用同一段代码，做的校验完全一样。
...
```bash
tar -xzf tgs-0.3.0-linux-amd64.tar.gz
cd tgs-0.3.0-linux-amd64
...
1. 面板 →「机器人」→ 该机器人的卡片上会直接显示**最近一次中继失败的原因**
   （没绑群 / 机器人不是管理员 / 群没开 Topics）。这条线索原先只写进容器日志，
   而现象是「用户发了消息，会话列表里却什么都没有」，看不出是哪里断了
2. 面板 →「机器人」→「群体检」—— 它会直接告诉你缺哪一项权限
3. 面板 →「审计」→「广告命中」—— 消息可能被规则拦了
4. 用户是否被静默 / 禁言 / 拉黑（会话详情页顶部会显示生效中的处罚）
5. 是否触发了刷屏阈值（默认 3 秒内超过 8 条）
...
没有运行时证据** —— 那需要真实的 BotFather token 与管理群。**管理机器人同理**：
它的放行判断、按钮回调、`/add` 对话流程都有单元测试覆盖，但「在真实 Telegram
