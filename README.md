...
- **系统规则随版本升级**：预置规则不只是首次播种。每次启动都会把代码里的定义同步进库，
  所以修一条预置规则的正则，**已经跑着的部署也拿得到**。三条边界：管理员停用过的不会被重新打开、
  改过的规则归管理员（代码不再接管）、删掉的不会复活。

#### 规则的两类锚点

预置规则原本锚定的是「**怎么联系到卖家**」：邀请链接、短链接、「加微信私我」。
但有一类广告通篇不写联系方式 —— 它卖的是犯罪工具本身，靠发帖人账号被私聊。
实测一条这类广告（盗 U / 钱包授权 / 远控 / 群发软件）对前三类规则的命中数是 **0**。
因此补了一组锚定「**犯罪工具与服务**」的规则：盗币黑话、钱包授权钓鱼、远控木马、
群发工具。同时按**精度分层**：拿不准的词（`一条龙`、`手把手教`、`助记词`）只做
弱信号告警，进删除 / 处罚档的必须是正常人不会写的说法 ——「一条龙服务」
是商家在说话，`盗U` / `假钱包` / `植入手机` 不是。

#### 多信号共现（`cooccurrence`）

单关键词规则永远滞后于对抗 —— 黑话换个说法就绕过了。但「一条消息里同时谈论
好几样犯罪工具」这个形态不会变。

这个模式**不匹配文本**，它数的是同一条消息上**命中了几条不同的规则**：
`pattern` 框里填的是一个整数阈值（例如 `3`），不是正则。

配套用法是「弱信号 + 共现」：拿不准的词（`远控`、`剪贴板`、`助记词`）写成
`notify` 动作的弱信号规则 —— 单独命中只告警、不删消息，因为「我的助记词丢了」
是受害者在求助 —— 再由共现规则把多个弱信号聚合成重罚。计数按**不同规则**算，
同一条规则命中三遍仍然只算一个信号。

它无法在沙盒里单条测试，面板上会直接说明这一点。规则列表与审计详情里也都按
阈值展示（「同一条消息命中 N 条不同规则时触发」），而不是套 `/3/` 那副正则的壳子 ——
命中记录连带匹配方式一起快照，否则事后回看只能把它读成一条匹配字面量「3」的正则。
...
> **写规则时的一个坑**：RE2 的 `\b` 是 **ASCII 词边界**（`\w` = `[0-9A-Za-z_]`）。
> 币种的常见写法是数字紧贴 ticker，而 `出1000usdt` 里 `0` 与 `u` 都是词字符，
> 中间没有边界 —— `\b(?:usdt|btc)\b` 会直接放行，**越是正常写法越漏**。
> 需要「左侧不受数字影响、但要挡住 `whether`/`method` 里的 `eth`」时，
> 用 `(?:^|[^A-Za-z])(?:usdt|btc|eth|trx)\b` 这种单侧前导边界。
