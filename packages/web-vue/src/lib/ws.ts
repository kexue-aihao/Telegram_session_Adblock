...
class WsClient {
  ...
  /** 重连退避：从 1s 开始翻倍，封顶 30s —— 长时间挂着时不该持续轰炸服务端 */
  ...
  private handleFrame(frame: WsFrame): void {
    // hello 的 seq 恒为 0（它是握手不是事件），不参与连续性判断
    if (frame.type === 'hello') {
      const payload = frame.payload as { serverStartedAt?: number };
      // 服务重启后 seq 会从头开始，此时本地的 seq 基线也要清零
      if (payload?.serverStartedAt && payload.serverStartedAt > this.serverStartedAt) {
