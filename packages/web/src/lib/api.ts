...
export async function request<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  try {
    response = await fetch(`${path}${query ? toQuery(query) : ''}`, {
      method,
      // same-origin + HttpOnly Cookie：不设 credentials 时 fetch 默认同源带 Cookie，
      // 显式写出来是为了让「这里依赖会话 Cookie」这件事在代码里可见
    ...
    throw new ApiError(0, '无法连接服务器，请检查服务是否正在运行', err);
  }
  ...
  if (!response.ok) {
    const payload = parsed as { error?: string; detail?: unknown } | null;
...
/** 带类型的便捷封装，省掉每处都写 `request<T>('...', {method:'POST'})` */
export const api = {
